/*
    ...
    Purpose: Create the verified dbo.usp_LoadFactSales procedure used by
             pipeline PL_SCD2_EndToEnd.
    ...
    Pipeline assumption:
    PL_SCD2_EndToEnd calls this procedure only after DF_SCD2_Staging_Live and
    dbo.usp_LoadDimCustomer_SCD2 succeed. Execute while connected to WH_SCD2.
    Existing keys must be valid, and executions must be serialized because
    MAX(SalesKey) surrogate-key generation is not concurrency-safe. OrderNumber
    filtering keeps unchanged pipeline reruns idempotent.
*/
